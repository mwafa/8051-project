...
;kal                       Allocated with name '_main_kal_2_3'
;------------------------------------------------------------
;	uts_1.c:19: void main(void)
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
	...
	ar1 = 0x01
	ar0 = 0x00
;	uts_1.c:21: unsigned int digit[4]    = {2,3,5,9}; // pukul 23.59
	mov	(_main_digit_1_2 + 0),#0x02
	mov	(_main_digit_1_2 + 1),#0x00
	mov	((_main_digit_1_2 + 0x0002) + 0),#0x03
	mov	((_main_digit_1_2 + 0x0002) + 1),#0x00
	mov	((_main_digit_1_2 + 0x0004) + 0),#0x05
	mov	((_main_digit_1_2 + 0x0004) + 1),#0x00
	mov	((_main_digit_1_2 + 0x0006) + 0),#0x09
	mov	((_main_digit_1_2 + 0x0006) + 1),#0x00
;	uts_1.c:22: unsigned int angka[10] = {0x0C0,0x0F9,0x0A4,0x0B0,0x99,0x92,0x82,0x0F8,0x80,0x90}; //data angka
...
;	uts_1.c:27: P0_7 = 1; // menyalakan Seven Segment
	setb	_P0_7
;	uts_1.c:32: for(kal = 0; kal<KALIBRATOR; kal++){
00133$:
	clr	a
	mov	_main_kal_2_3,a
	mov	(_main_kal_2_3 + 1),a
00123$:
;	uts_1.c:33: k = 0;
	clr	a
	mov	_main_k_1_2,a
	mov	(_main_k_1_2 + 1),a
;	uts_1.c:34: for(i= 1; i>=0 ; i--){
	mov	_main_i_1_2,#0x01
;	1-genFromRTrack replaced	mov	(_main_i_1_2 + 1),#0x00
	mov	(_main_i_1_2 + 1),a
;	uts_1.c:35: for(j = 1; j>=0; j--){
00129$:
	mov	r6,#0x01
	mov	r7,#0x00
	mov	r4,_main_k_1_2
...
00118$:
	...
	orl	a,r3
	jnz	00118$
;	uts_1.c:43: P1 = 0xFF; // memadamkan sevent segment sebelum berganti digit.
	mov	_P1,#0xff
;	uts_1.c:35: for(j = 1; j>=0; j--){
	dec	r6
	cjne	r6,#0xff,00171$
...
;	uts_1.c:32: for(kal = 0; kal<KALIBRATOR; kal++){
	inc	_main_kal_2_3
	clr	a
	cjne	a,_main_kal_2_3,00175$
	inc	(_main_kal_2_3 + 1)
00175$:
	clr	c
	mov	a,_main_kal_2_3
	subb	a,#0x3c
	mov	a,(_main_kal_2_3 + 1)
	xrl	a,#0x80
	subb	a,#0x80
	jnc	00176$
	ljmp	00123$
...
00110$:
;	uts_1.c:61: if(digit[0]*10 + digit[1] >= 24){
